...
```bash
npx instagram-cta-kit init --dir ./instagram-cta --locale en --mode polling
```

3. Tell the user to put secrets directly into `.env`. Do not ask them to paste secrets into chat.
4. Run `npx instagram-cta-kit doctor --dir ./instagram-cta`.
5. Add a route:
...
```bash
npx instagram-cta-kit routes validate --dir ./instagram-cta --check-guides
npx instagram-cta-kit start --dir ./instagram-cta
```
